// server.mjs — ESM + Codespaces friendly
...
function getJobStatus(job) {
  ...
  if (elapsed <= 0) {
    ...
    stage = 'VALIDATING';
    percent = Math.max(1, Math.floor((elapsed / (validating + processing)) * 100 * 0.4)); // ~40%
...
// serve from repo root so index.html/assets resolve
...
app.get('/api/system/report-field-specs', async (req, res) => {
  try {
    ...
    const keys = String(keysParam).split(',').map(s => s.trim()).filter(Boolean);
    const subset = {};
...
app.get('/api/jobs/:jobId/results', (req, res) => {
  ...
  const reports = [
    { key: 'period_end_soi', human_name: 'Period End SOI', filename: 'Period-End SOI.csv',
      downloadUrl: `/downloads/${job.id}/Period-End%20SOI.csv` },
    { key: 'prior_period_end_soi', human_name: 'Prior Period End SOI', filename: 'Prior Period-End SOI.csv',
      downloadUrl: `/downloads/${job.id}/Prior%20Period-End%20SOI.csv` },
    { key: 'purchases_and_sales_report', human_name: 'Purchases & Sales Report', filename: 'P&S Report.csv',
...
app.get('/api/detection/:jobId/status', async (req, res) => {
  try {
    ...
    if (!snap) return res.status(404).json({ ok:false, error:'Unknown jobId' });
...
app.listen(PORT, HOST, () => {
  console.log(`APEX UI server running at http://${HOST}:${PORT}`);
});
